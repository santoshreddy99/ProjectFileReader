...
public class ProjectFileReader {
	...
	HashMap<String, HashMap<String, Integer>> wordFrequencyByDocumentMap = new HashMap<String, HashMap<String, Integer>>();
	...
	public static void main(String[] args) {
		...
		System.out.println("Total time to index collection (in ms) : "+(endTime-startTime));
		
		startTime = System.currentTimeMillis();
		reader.evaluateFormulae();
	...
	public void readDocuments(){
		int totalFilteredWords = 0;//counter to calculate total words after stemmer applied and stop words excluded
		int totalNonFilteredWords = 0;//counter to calculate total words before stemmer applied and stop words included
		String word = null;
		try{
		    ...
		    while (docIterator.hasNext()) {
		      ...
		      //documentText = documentText.replaceAll("[^a-zA-Z0-9]", "");//if numbers should not be skipped
		        ...
		        wordFrequencyByDocumentMap.put(docName, eachDocWordCollection);
		...
		System.out.println("Document size"+wordFrequencyByDocumentMap.size());
		System.out.println("Total Words (including stop words and no stemmer is applied) : "+totalNonFilteredWords);
		System.out.println("Total Words (excluding stop words and after stemmer is applied) : "+totalFilteredWords);
	    ...
	    while (wordMapIterator.hasNext()) {
	        ...
	        //Get the occurance of string docName which is posting of that word
	        while ((idx = value.indexOf("docName", idx)) != -1)
	        {
	           idx++;
	           count++;
	        }
	        //to get total posting length
	        totalPostingLength = totalPostingLength+count;
	        if(shortestposting == 0 || count < shortestposting){
	    ...
	    System.out.println("Total posting length is "+totalPostingLength);
	    System.out.println("Average posting length is "+totalPostingLength/wordFrequencyMap.size());
	...
	public void evaluateFormulae(){
		Scanner input = new Scanner(System.in);
		String inputExpr;
		String tempInputExpr = null;
		 
	      System.out.println("Enter a input formula :");
	      inputExpr = input.nextLine();
	      
	      long startTime= System.currentTimeMillis();
	      
	      String inputText = inputExpr.replaceAll("[^a-zA-Z]", " ");
	      
	      String []docWords=inputText.split(" ");
	      double wordValue= 0.0;
	      String word = null;
	      String docName = null;
	      HashMap<String, Integer> tempHashMap = null;
	      
	      HashMap<String, Double> RSVValuesByDocumentMap = new HashMap<String, Double>();
	      Iterator docIterator = wordFrequencyByDocumentMap.keySet().iterator();
		    while (docIterator.hasNext()) {
		    	Double exprOutput;
		    	docName = (String)docIterator.next(); 
		    	tempHashMap = wordFrequencyByDocumentMap.get(docName);
		    	
		    	tempInputExpr = inputExpr;
		    	
		    	HashMap<String, Double> wordExprValuesMap = new HashMap<String, Double>();
		    	String stemmeredWord = null;
				for(int i=0; i<docWords.length;i++) {
					word = docWords[i].trim();
			        ...
			        stemmerObject.add(word.toCharArray(), word.length());
			        stemmerObject.stem();
			        stemmeredWord = stemmerObject.toString();
			        int stemmeredValue = 0;
			        if(tempHashMap.get(stemmeredWord) !=null )
			        	stemmeredValue = tempHashMap.get(stemmeredWord);
			        
			        wordValue = (double)stemmeredValue/getHigestFreqinDoc(tempHashMap);
			        
			        wordExprValuesMap.put(word, wordValue);
			        
			        //replace each word in expression
			        tempInputExpr = tempInputExpr.replaceAll(word, String.valueOf(wordValue));
					
				}
				
				//Get the RSV value for this document and store it to a map
				exprOutput =  new ExtendedDoubleEvaluator().evaluate(tempInputExpr);
				//System.out.println (inputExpr +" for document "+ docName+" : "+exprOutput);
				RSVValuesByDocumentMap.put(docName, exprOutput);
				
		    }
		     
		    
		   List<Map.Entry<String, Double>> entries = sortEntries(RSVValuesByDocumentMap); 
		   System.out.println("Printing top 20 RSV values Data ::::" );
		   
		 //To print top 20 RSV values and their doc Names
	        int counter = 1;
		   for(int i=entries.size(); i>0;i--){
			   if(counter > 20)
		    		break;
			   Map.Entry<String, Double> entry = entries.get(i-1);
			   System.out.println(counter +" :  RSV Value :"+entry.getValue() +" Document Name : "+entry.getKey());
			   System.out.println("Document Text :" );
			   System.out.println(documentsMap.get(entry.getKey()) );
			   counter++;
			   
		   }
		   
		 //To print top doc Names with non-zero RSV values
		   List<String> nonZeroRSVDocList = new ArrayList<String>();
		   for(int i=0; i<entries.size();i++){
			   Map.Entry<String, Double> entry = entries.get(i);
			   if(entry.getValue() > 0)
		    		nonZeroRSVDocList.add(entry.getKey());
			   
		   }
	       
	       
		   System.out.println("No.of Documents with non-zero RSV values : "+nonZeroRSVDocList.size()); 
		   System.out.println("Documents with non-zero RSV values : "+nonZeroRSVDocList);
		    
		   long endTime = System.currentTimeMillis();
		    
		   System.out.println("Time taken for the search (in ms) : "+(endTime-startTime));
		    
	}
	
	public int getHigestFreqinDoc(HashMap<String, Integer> docFreqMap){
		int higestFreq = 1;
		String word = null;
		Iterator wordIterator = docFreqMap.keySet().iterator();
	    while (wordIterator.hasNext()) {
	    	word = (String)wordIterator.next(); 
	    	if(docFreqMap.get(word) > higestFreq){
	    		higestFreq = docFreqMap.get(word);
	    	}
	    }
	...
	public List sortEntries(HashMap RSVValuesByDocumentMap)
	{
		List<Map.Entry<String, Double>> entries = new ArrayList<Map.Entry<String, Double>>(RSVValuesByDocumentMap.entrySet());
	...
	/*class HashMapValuesComparator implements Comparator {
	    HashMap<String, Double> inputMap;

	    public HashMapValuesComparator(HashMap inputMap) {
	        this.inputMap = inputMap;
	    }

		@Override
		public int compare(Object arg0, Object arg1) {
			if (inputMap.get(arg0.toString()) >= inputMap.get(arg1.toString())) {
	            return -1;
	        } else {
	            return 1;
	        }
